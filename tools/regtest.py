#!/usr/bin/env python3

from argparse import ArgumentParser
import os
import json
from pathlib import Path
from typing import Tuple

script_dir = os.path.dirname(os.path.realpath(__file__))
root_dir = os.path.abspath(os.path.join(script_dir, ".."))


UNIT_KEYS_TO_DIFF = [
    "fuzzy_match_percent",
    "matched_code_percent",
    "matched_data_percent",
    "complete_code_percent",
    "complete_data_percent",
]

FUNCTION_KEYS_TO_DIFF = [
    "fuzzy_match_percent",
]

type Regression = Tuple[str, str, float, float]


def get_regressions(changes_file: str) -> list[Regression]:
    changes_file = os.path.relpath(changes_file, root_dir)
    with open(changes_file, "r") as f:
        changes = json.load(f)

    regressions = []
    progressions = []

    def diff_key(object_name: str, object: dict, key: str):
        nonlocal regressions
        from_value = object.get("from", {}).get(key, 0.0)
        to_value = object.get("to", {}).get(key, 0.0)
        key = key.removesuffix("_percent")
        change = (object_name, key, from_value, to_value)
        if from_value > to_value:
            regressions.append(change)
        elif to_value > from_value:
            progressions.append(change)

    for key in UNIT_KEYS_TO_DIFF:
        diff_key(None, changes, key)

    for unit in changes.get("units", []):
        unit_name = unit["name"]
        for key in UNIT_KEYS_TO_DIFF:
            diff_key(unit_name, unit, key)
        # Ignore sections
        for func in unit.get("functions", []):
            func_name = func["name"]
            for key in FUNCTION_KEYS_TO_DIFF:
                diff_key(func_name, func, key)

    return regressions, progressions


def generate_regressions_plaintext(regressions: list[Regression]) -> str:
    if len(regressions) == 0:
        return ""

    name_max_len = max(len(name or "Total") for name, _, _, _ in regressions)
    key_max_len = max(len(key) for _, key, _, _ in regressions)
    name_max_len = min(100, name_max_len)

    out_lines = []
    for name, key, from_value, to_value in regressions:
        if name is None:
            name = "Total"
        if len(name) > name_max_len:
            name = name[:name_max_len]
        out_lines.append(
            f"{name:>{name_max_len}} | {key:<{key_max_len}} | {from_value:6.2f}% -> {to_value:5.2f}%"
        )

    return "\n".join(out_lines)


def generate_regressions_markdown(
    regressions: list[Regression], description: str
) -> str:
    if len(regressions) == 0:
        return ""

    out_lines = []
    name_max_len = 100

    out_lines.append("<details>")
    out_lines.append(
        f"<summary>Detected {len(regressions)} {description} compared to the base:</summary>"
    )
    out_lines.append("")  # Must include a blank line before a table
    out_lines.append("| Name | Type | Before | After |")
    out_lines.append("| ---- | ---- | ------ | ----- |")

    for name, key, from_value, to_value in regressions:
        if name is None:
            name = "Total"
        else:
            if len(name) > name_max_len:
                name = name[: name_max_len - len("...")] + "..."
            name = f"`{name}`"  # Surround with backticks
        key = key.replace("_", " ").capitalize()
        out_lines.append(f"| {name} | {key} | {from_value:.2f}% | {to_value:.2f}% |")

    out_lines.append("</details>")

    return "\n".join(out_lines)


def main():
    parser = ArgumentParser(description="Format report diff regressions.")
    parser.add_argument(
        "report_diff_file",
        type=Path,
        help="""path to the JSON file containing the changes, generated by objdiff-cli.""",
    )
    parser.add_argument(
        "-o",
        "--output",
        type=Path,
        help="""Output file (prints to console if unspecified)""",
    )
    parser.add_argument(
        "--all",
        action="store_true",
        help="""Includes progressions as well.""",
    )
    args = parser.parse_args()

    regressions, progressions = get_regressions(args.report_diff_file)

    if args.output:
        markdown_output = generate_regressions_markdown(regressions, "regressions")
        if args.all:
            markdown_output += generate_regressions_markdown(
                progressions, "progressions"
            )
        with open(args.output, "w", encoding="utf-8") as f:
            f.write(markdown_output)
    else:
        if args.all:
            regressions = progressions + regressions
        text_output = generate_regressions_plaintext(regressions)
        print(text_output)


if __name__ == "__main__":
    main()
